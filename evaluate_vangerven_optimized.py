#!/usr/bin/env python3
"""
🧠 Evaluate Optimized Vangerven Model
Comprehensive evaluation of the optimized Vangerven model with advanced metrics.
"""
...
def load_optimized_vangerven_model(checkpoint_path, device='cuda'):
    """Load the optimized Vangerven model with all components."""
    
    print(f"📁 Loading optimized Vangerven model from: {checkpoint_path}")
    ...
    print(f"✅ Optimized Vangerven model loaded successfully!")
    return model, decoder, criterion

def evaluate_and_compare_vangerven():
    """Evaluate optimized Vangerven model and compare with baseline."""
    
    print("🧠 OPTIMIZED VANGERVEN MODEL EVALUATION")
    print("=" * 50)
    ...
    opt_model, opt_decoder, opt_criterion = load_optimized_vangerven_model(
        "checkpoints/best_vangerven_optimized_model.pt", device
    )
    
    if opt_model is None:
        print("❌ Please train the optimized Vangerven model first:")
        print("   PYTHONPATH=src python3 train_vangerven_optimized.py")
        return
    
    # Load baseline model for comparison
    baseline_checkpoint_path = "checkpoints/best_vangerven_simple_model.pt"
    baseline_model = None
    baseline_decoder = None
    
    if Path(baseline_checkpoint_path).exists():
        print(f"📁 Loading baseline Vangerven model for comparison...")
        ...
        baseline_model = ImprovedBrainLDM(
            fmri_dim=3092, image_size=28, guidance_scale=7.5
        ).to(device)
        ...
        print(f"✅ Baseline Vangerven model loaded for comparison")
    ...
    # Print results
    print(f"\n📈 VANGERVEN EVALUATION RESULTS")
    print("=" * 35)
    
    print(f"🎯 Optimized Vangerven Model:")
    ...
    if baseline_metrics is not None:
        ...
        print(f"\n📊 VANGERVEN IMPROVEMENT ANALYSIS:")
    ...
    create_vangerven_comparison_visualization(
...
def create_vangerven_comparison_visualization(targets, opt_recons, baseline_recons, labels, 
                                            opt_metrics, baseline_metrics):
    """Create comprehensive comparison visualization for Vangerven."""
    
    print(f"\n🎨 Creating Vangerven optimization visualization...")
    ...
    fig.suptitle('Optimized vs Baseline Vangerven Reconstruction Comparison', 
    ...
    # Add row labels
    axes[0, 0].text(-0.15, 0.5, 'Target\nDigit', rotation=90, va='center', ha='center', 
    ...
    # Save visualization
    output_path = "results/vangerven_optimization_comparison.png"
    Path("results").mkdir(exist_ok=True)
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"💾 Vangerven comparison saved: {output_path}")
    
    plt.show()

def main():
    """Main evaluation function for optimized Vangerven."""
    
    opt_metrics, baseline_metrics = evaluate_and_compare_vangerven()
    
    print(f"\n🎯 VANGERVEN OPTIMIZATION EXPERIMENT SUMMARY")
    print("=" * 45)
    ...
    if baseline_metrics is not None:
        ...
        if improvements:
            ...
            print(f"   💡 Vangerven baseline was already quite good")
    
    print(f"\n📁 Results saved to: results/vangerven_optimization_comparison.png")
