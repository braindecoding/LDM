#!/usr/bin/env python3
"""
🧠 Show Vangerven Results - Target vs Reconstruction
Evaluation and visualization script for Brain LDM with Vangerven dataset.
"""
...
def main():
    """Main function to show vangerven reconstruction results."""
    
    print("🧠 VANGERVEN DATASET RECONSTRUCTION VISUALIZATION")
    print("=" * 55)
    ...
    if not Path(checkpoint_path).exists():
        ...
        print("   PYTHONPATH=src python3 train_vangerven_simple.py")
        return
    ...
    fig.suptitle('Brain LDM: fMRI → Image Reconstruction (Vangerven Dataset)', 
    ...
    # Save visualization
    output_path = "results/vangerven_reconstruction_results.png"
    ...
    for i in range(num_samples):
        ...
        print(f"   Sample {i+1} (Digit {labels[i]}): MSE={sample_mse:.6f}, Corr={sample_corr:.6f}")
    
    plt.show()
    
    print(f"\n🎉 Vangerven reconstruction visualization complete!")
    print(f"📁 Results saved to: {output_path}")
    print(f"🔍 The visualization shows:")
    print(f"   • Top row: Original digit stimulus images")
    ...
    print(f"\n📊 Dataset Information:")
    print(f"   • Dataset: Vangerven (digit69_28x28.mat)")
