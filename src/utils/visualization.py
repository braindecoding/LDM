...
def plot_metrics_comparison(
    ...
    save_path: Optional[str] = None,
    title: str = "Reconstruction Metrics Comparison"
) -> plt.Figure:
    """
    Plot comprehensive metrics comparison including advanced metrics.
    ...
    key_metrics = {
        ...
        'Spatial Metrics': [
            'val_spatial_pattern_correlation',
            'val_spatial_pattern_mse'
        ]
    }

    # Plot each category
    for idx, (category, metric_names) in enumerate(key_metrics.items()):
        row = idx // 3
        col = idx % 3
        ...
        if row < 2 and col < 3:
            ...
            for metric in metric_names:
                if metric in metrics and not np.isnan(metrics[metric]) and not np.isinf(metrics[metric]):
                    available_metrics.append(metric.replace('val_', '').replace('_', ' ').title())
                    values.append(metrics[metric])

            if available_metrics and values:
                # Create bar plot
                bars = ax.bar(range(len(available_metrics)), values, alpha=0.7)
...
def plot_individual_stimulus_reconstruction_images(
    original: np.ndarray,
    reconstructed: np.ndarray,
    num_samples: int = 5,
    save_path: Optional[str] = None,
    title: str = "Individual Stimulus vs Reconstruction Comparison"
) -> plt.Figure:
    """
    Plot stimulus vs reconstruction as 2D images side by side in a clean grid format.
    ...
    fig = plt.figure(figsize=(6, 2.5 * num_samples))
    fig.suptitle(title, fontsize=16, fontweight='bold')

    # Create grid layout
    gs = fig.add_gridspec(num_samples, 2, hspace=0.3, wspace=0.1)

    # Add column headers
    fig.text(0.25, 0.95, 'Stimulus', ha='center', va='center', fontsize=14, fontweight='bold')
    fig.text(0.75, 0.95, 'Reconstruction', ha='center', va='center', fontsize=14, fontweight='bold')

    for i in range(num_samples):
        # Convert 1D fMRI data to 2D image
        orig_data = original[i]
        recon_data = reconstructed[i]

        # Find appropriate 2D dimensions (try to make square-ish)
        n_voxels = len(orig_data)
        ...
        if pad_size > 0:
            orig_padded = np.pad(orig_data, (0, pad_size), mode='constant', constant_values=0)
            recon_padded = np.pad(recon_data, (0, pad_size), mode='constant', constant_values=0)
        else:
            orig_padded = orig_data
            recon_padded = recon_data

        # Reshape to 2D
        orig_2d = orig_padded.reshape(height, width)
        recon_2d = recon_padded.reshape(height, width)
        ...
        # Plot stimulus (original)
        im1 = ax1.imshow(orig_norm, cmap='gray', aspect='equal', interpolation='nearest', vmin=0, vmax=1)
        ax1.set_ylabel(f'Image {i+1}', fontsize=12, fontweight='bold')
        ax1.set_xticks([])
        ax1.set_yticks([])
        ...
        # Plot reconstruction
        im2 = ax2.imshow(recon_norm, cmap='gray', aspect='equal', interpolation='nearest', vmin=0, vmax=1)
        ax2.set_xticks([])
        ax2.set_yticks([])
        ...
        if np.std(orig_data) > 1e-8 and np.std(recon_data) > 1e-8:
            corr, _ = stats.pearsonr(orig_data, recon_data)
        else:
            corr = 0.0

        # Add correlation text on reconstruction
    ...
    if save_path:
        ...
        logger.info(f"Individual stimulus-reconstruction images saved to {save_path}")

    return fig


def plot_stimulus_reconstruction_comparison(
    original: np.ndarray,
    reconstructed: np.ndarray,
    num_samples: int = 6,
    save_path: Optional[str] = None,
    title: str = "Stimulus vs Reconstruction Comparison"
) -> plt.Figure:
    """
    Plot detailed comparison between stimulus (original) and reconstruction.
    ...
    for i in range(num_samples):
        # Original signal
        axes[i, 0].plot(original[i], alpha=0.8, linewidth=1, color='blue', label='Original')
        axes[i, 0].set_title(f'Sample {i+1}: Original Stimulus')
        axes[i, 0].set_xlabel('Voxel Index')
        axes[i, 0].set_ylabel('Activation')
        axes[i, 0].grid(True, alpha=0.3)
        axes[i, 0].legend()
        ...
        axes[i, 1].plot(reconstructed[i], alpha=0.8, linewidth=1, color='orange', label='Reconstructed')
        ...
        # Overlay comparison
        axes[i, 2].plot(original[i], alpha=0.7, linewidth=1, color='blue', label='Original')
        axes[i, 2].plot(reconstructed[i], alpha=0.7, linewidth=1, color='orange', label='Reconstructed')
        axes[i, 2].set_title(f'Sample {i+1}: Overlay Comparison')
        axes[i, 2].set_xlabel('Voxel Index')
        axes[i, 2].set_ylabel('Activation')
        axes[i, 2].grid(True, alpha=0.3)
        axes[i, 2].legend()

        # Compute correlation for this sample
        if np.std(original[i]) > 1e-8 and np.std(reconstructed[i]) > 1e-8:
        ...
        axes[i, 3].plot(error, alpha=0.8, linewidth=1, color='red', label='Error')
        axes[i, 3].axhline(y=0, color='black', linestyle='--', alpha=0.5)
        axes[i, 3].set_title(f'Sample {i+1}: Error (r={corr:.3f})')
        axes[i, 3].set_xlabel('Voxel Index')
        axes[i, 3].set_ylabel('Error')
        ...
        axes[i, 3].text(0.02, 0.98, f'MSE: {mse:.4f}\nMAE: {mae:.4f}',
                       transform=axes[i, 3].transAxes, va='top',
                       bbox=dict(boxstyle="round", facecolor='white', alpha=0.8))
    ...
    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Stimulus-reconstruction comparison saved to {save_path}")

    return fig


def plot_voxel_activation_heatmap(
    original: np.ndarray,
    reconstructed: np.ndarray,
    save_path: Optional[str] = None,
    title: str = "Voxel Activation Heatmap Comparison"
) -> plt.Figure:
    """
    Plot heatmap comparison of voxel activations.
    ...
    max_samples = min(20, original.shape[0])
    orig_subset = original[:max_samples]
    recon_subset = reconstructed[:max_samples]

    # Original heatmap
    im1 = axes[0, 0].imshow(orig_subset, aspect='auto', cmap='viridis', interpolation='nearest')
    ...
    im2 = axes[0, 1].imshow(recon_subset, aspect='auto', cmap='viridis', interpolation='nearest')
    axes[0, 1].set_title('Reconstructed Activations')
    axes[0, 1].set_xlabel('Voxel Index')
    axes[0, 1].set_ylabel('Sample Index')
    plt.colorbar(im2, ax=axes[0, 1])

    # Difference heatmap
    diff = orig_subset - recon_subset
    im3 = axes[1, 0].imshow(diff, aspect='auto', cmap='RdBu_r', interpolation='nearest')
    axes[1, 0].set_title('Difference (Original - Reconstructed)')
    axes[1, 0].set_xlabel('Voxel Index')
    axes[1, 0].set_ylabel('Sample Index')
    plt.colorbar(im3, ax=axes[1, 0])

    # Correlation heatmap (sample-wise correlations)
    correlations = []
    for i in range(max_samples):
        if np.std(orig_subset[i]) > 1e-8 and np.std(recon_subset[i]) > 1e-8:
            corr, _ = stats.pearsonr(orig_subset[i], recon_subset[i])
    ...
    im4 = axes[1, 1].imshow(corr_matrix, aspect='auto', cmap='coolwarm',
                           vmin=-1, vmax=1, interpolation='nearest')
    axes[1, 1].set_title('Sample-wise Correlations')
    ...
    for i, corr in enumerate(correlations):
        axes[1, 1].text(0, i, f'{corr:.2f}', ha='center', va='center',
    ...
    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Voxel activation heatmap saved to {save_path}")
...
def create_visualization_report(
    ...
    """
    Create a comprehensive visualization report with advanced metrics and comparisons.
    ...
    # Metrics comparison
    if metrics:
        fig = plot_metrics_comparison(metrics, save_path=save_dir / "metrics_comparison.png")
        plt.close(fig)
        saved_files['metrics_comparison'] = str(save_dir / "metrics_comparison.png")

    # Individual Stimulus vs Reconstruction images (side by side)
    fig = plot_individual_stimulus_reconstruction_images(
        original, reconstructed, save_path=save_dir / "individual_stimulus_reconstruction_images.png"
    )
    plt.close(fig)
    saved_files['individual_stimulus_reconstruction_images'] = str(save_dir / "individual_stimulus_reconstruction_images.png")

    # Stimulus vs Reconstruction comparison (detailed)
    fig = plot_stimulus_reconstruction_comparison(
        original, reconstructed, save_path=save_dir / "stimulus_reconstruction_comparison.png"
    )
    plt.close(fig)
    saved_files['stimulus_reconstruction_comparison'] = str(save_dir / "stimulus_reconstruction_comparison.png")

    # Voxel activation heatmap
    fig = plot_voxel_activation_heatmap(
        original, reconstructed, save_path=save_dir / "voxel_activation_heatmap.png"
    )
    plt.close(fig)
    saved_files['voxel_activation_heatmap'] = str(save_dir / "voxel_activation_heatmap.png")

    # Original reconstructions plot
