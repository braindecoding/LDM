...
class LatentDiffusionModel(nn.Module):
    ...
    def reconstruct_fmri(self, x: torch.Tensor, use_diffusion: bool = True) -> torch.Tensor:
        """
        Reconstruct fMRI data through the complete Latent Diffusion pipeline.

        Args:
            x: Input fMRI data
            use_diffusion: Whether to use diffusion process (True for LDM, False for VAE-only)
        ...
        if use_diffusion:
            ...
            # Step 2: Add noise to latents (simulate degradation)
            batch_size = latents.shape[0]
            device = latents.device

            # Add moderate noise (not full noise like in generation)
            noise_level = 0.3  # Adjustable parameter
            noise = torch.randn_like(latents) * noise_level
            noisy_latents = latents + noise

            # Step 3: Use diffusion model to denoise (partial denoising)
            num_denoising_steps = 10  # Fewer steps for reconstruction vs generation
            timesteps = torch.linspace(
            ...
            for timestep in timesteps:
                with torch.no_grad():
                    ...
                    if timestep > 0:
                        ...
                        denoised_latents = (denoised_latents - (1 - alpha_prod).sqrt() * noise_pred) / alpha_prod.sqrt()

            # Step 4: Decode denoised latents back to fMRI
            reconstruction = self.decode_from_latent(denoised_latents)
